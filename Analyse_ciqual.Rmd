...
library(readxl)
url <- "Table Ciqual 2020_FR_2020 07 07.xls"
t<- read_excel(url, guess_max = 3000)
...
```{r}
## Cette fonction permet de gérer les données mal renseignés et les données manquante dans le jeu de données 
recod<- function(d){
  d<- gsub("[-]", NA, d)
  d <- gsub(" ", NA, d)
  d <- gsub(",", ".", d)
...
## utilisation de la fonction pour transformation des variables qui en ont besoin 
...
combien y a til de dessert pour les quels l'energie n'est pas connue?
...
##(on a 272 qui ont l'energie inconnue )
summary(V$Energie)
## ou 
...
Histogramme du contenu en sucres des desserts 
```{r}
hist(V$Sucres, xlab= "Sucre (g/100g)", ylab= "Densité", main="Histogramme du contenu en sucres")
```

boxplot du contenu en sucres selon le groupe du dessert
